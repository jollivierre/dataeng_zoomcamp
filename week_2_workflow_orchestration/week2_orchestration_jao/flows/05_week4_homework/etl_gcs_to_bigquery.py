...
@task()
def write_bigquery(df: pd.DataFrame, taxi_colour: str) -> None:
    """Write parquet file to BigQuery"""
    
    gcp_credentials_block = GcpCredentials.load("dataeng-gcp-creds-jao")

    df.to_gbq(
        destination_table=f"trips_data_all.{taxi_colour}_tripdata",
...
def query_tripdata(taxi_colour: str) -> pd.DataFrame:
    """Write BigQuery data to DataFrame"""
    
    gcp_credentials_block = GcpCredentials.load("dataeng-gcp-creds-jao")
    bq_sql = f"SELECT count(*) as row_count FROM trips_data_all.{taxi_colour}_tripdata"
...
def etl_parent_flow(
    ...
    df_bq_data = query_tripdata(taxi_colour)
